...
const
  ...
  M_Add('============================================================');
  M_Add('||                                                        ||');
  M_Add('||             Институт органического синтеза             ||');
  M_Add('||                  им. И.Я Постовского                   ||');
  M_Add('||                        УрО РАН                         ||');
  M_Add('||                                                        ||');
  M_Add('============================================================');
  M_Add('');
  M_Add('Параметры отчёта:');
  M_Add(' - Исходные данные:      '+Config.ReadString('General','Title','ERROR!!!'));
  M_Add(' - Диапазон дат:         '+
    DateToStr(DateFrom.Date)+' — '+DateToStr(DateTo.Date));
  M_Add(' - Дата создания отчёта: '+DateTimeToStr(Now));
  M_Add('');
  M_Add('');
  end;
  ...
  begin
  //Запрос на общее количество спектров
  ...
  M_Add(' - Зарегистрировано спектров:           '+IntToStr(SpectraReg));
  end;
  ...
  begin
  M_Add('============================================================');
  M_Add('                Общая информация по прибору:                ');
  M_Add('');
  M_Add('За рассматриваемый периодбыли выполнены следующие работы:   ');
  ...
  begin
  M_Add('============================================================');
  M_Add('                Общее время выполнения услуг:               ');
  M_Add('');
  ...
  begin
  M_Add('============================================================');
  M_Add('               Общее число выполненных услуг:               ');
  M_Add('');
  ...
  begin
  M_Add('============================================================');
  M_Add('               Время выполнения одной услуги:               ');
  M_Add('');
  ...
  var
    ...
    if NIR.Count=0 then
      begin
      NIR.Free;
      M_Add('!!!  НИР НЕ ОБНАРУЖЕНЫ!');
      exit;
      end;

    M_Add(' - НИР организации');
    ...
    for I2 := 0 to NIR.Count-1 do
      ...
      S1:='   - '+NIR.FieldAsString(NIR.FieldIndex['Title']);
        repeat
